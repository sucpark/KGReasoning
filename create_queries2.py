...
def ground_queries(dataset, query_structure, ent_in, ent_out, small_ent_in, small_ent_out, gen_num, max_ans_num,
                   query_name, mode, ent2id, rel2id, gen_all_queries):
    ...
    if query_structure == ['e', ['r']]:
        if gen_train:
            if gen_all_queries:
                ...
                write_links(dataset, train_ent_out, defaultdict(lambda: defaultdict(set)), max_ans_num,
                            'train-' + query_name)
        if gen_valid:
            if gen_all_queries:
