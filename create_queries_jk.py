...
def ground_queries(dataset, query_structure, ent_in, ent_out, small_ent_in, small_ent_out, gen_num, max_ans_num, query_name, mode, ent2id, rel2id, gen_all_queries):
    ...
    if gen_all_queries:
        # if mode == 'train' and query_name in ['2in', '3in', 'pin', 'pni', 'inp']:
        #     gen_num = gen_num / 10
...
def generate_queries(dataset, query_structures, gen_num, max_ans_num, gen_train, gen_valid, gen_test, query_names, save_name, gen_all_queries):
    ...
    if gen_valid:
        valid_queries, valid_tp_answers, valid_fp_answers, valid_fn_answers = ground_queries(dataset, query_structure, 
            valid_ent_in, valid_ent_out, train_ent_in, train_ent_out, gen_num[1], max_ans_num, query_name, 'valid', ent2id, rel2id, gen_all_queries)
    if gen_test:
        test_queries, test_tp_answers, test_fp_answers, test_fn_answers = ground_queries(dataset, query_structure, 
            test_ent_in, test_ent_out, valid_ent_in, valid_ent_out, gen_num[2], max_ans_num, query_name, 'test', ent2id, rel2id, gen_all_queries)
...
def main(dataset, seed, gen_train_num, gen_valid_num, gen_test_num, max_ans_num, reindex, gen_train, gen_valid, gen_test, gen_id, gen_all_queries, save_name, index_only):
    ...
    #                     [[e, [r, r, n]], [e, [r]]],
    #                    ]
    # query_names = ['1p', '2p', '3p', '2i', '3i', '2in', '3in', 'inp', 'pin', 'pni']

    if gen_all_queries:
        print("1p")
        generate_queries(dataset, query_structures[0:1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[0:1], save_name, gen_all_queries)
        time.sleep(1)
        ...
        for i in range(1, len(query_structures)):
            generate_queries(dataset, query_structures[i:i+1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[i:i+1], save_name, gen_all_queries)
    else:
        generate_queries(dataset, query_structures[gen_id:gen_id+1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[gen_id:gen_id+1], save_name, gen_all_queries)
